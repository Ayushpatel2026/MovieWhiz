  ...
  async notifyExperts(inputs: Input[]): Promise<ExpertResponse[]> {
    const promises: Promise<ExpertResponse>[] = inputs.map(async (input) => {
      if (input.type === "text") {
        ...
        return expert
          ? await expert.analyze(input)
          : Promise.resolve({ expertName: "LLM Expert", movieConfidences: [] });
      }

      if (input.type === "audio") {
        const expert = this.experts.find(
          (exp) => exp.name === "Soundtrack Expert"
        );
        return expert
          ? await expert.analyze(input)
          : Promise.resolve({
              expertName: "Soundtrack Expert",
              movieConfidences: [],
            });
      }
      ...
      if (input.type === "form") {
        const expert = this.experts.find(
          (exp) => exp.name === "Database Expert"
        );
        const response = expert
          ? await expert.analyze(input)
          : { expertName: "Database Expert", movieConfidences: [] };
        return response;
      }
